...
//--------------------------------------------------------------------------
HWND VeMobileSim::GetWindowHandle()
{
	if(m_pkWindow)
	{
...
//--------------------------------------------------------------------------
VE_BOOL VeMobileSim::GetDisplayMode(LPCSTR deviceName, DWORD index,
...
//--------------------------------------------------------------------------
VE_BOOL VeMobileSim::AddDisplay(LPSTR DeviceName)
{
...
//--------------------------------------------------------------------------
VeDWord VeMobileSim::GetWindowStyle(Window* pkWindow)
{
	DWORD dwStyle = 0;
...
//--------------------------------------------------------------------------
static bool s_bTouched = false;
//--------------------------------------------------------------------------
LRESULT CALLBACK VeMobileSim::WndProc(HWND hWnd, UINT uMessage, WPARAM wParam,
	LPARAM lParam)
{
	if(((VeMobileSim*)g_pDevice)->IMEHandleMessage(hWnd, uMessage, wParam, lParam))
		return 0;
	...
	case WM_SHOWWINDOW:
		{
			if(wParam) ((VeMobileSim*)g_pDevice)->Show();
			else ((VeMobileSim*)g_pDevice)->Hide();
		}
		break;
	case WM_ACTIVATE:
		if(LOWORD(wParam) != WA_INACTIVE)
		{
			((VeMobileSim*)g_pDevice)->Active();
		}
		else if(LOWORD(wParam) == WA_INACTIVE)
		{
			((VeMobileSim*)g_pDevice)->Inactive();
		}
		break;
	case WM_LBUTTONDOWN:
		{
			...
			pkParam->x = (VeFloat32)LOWORD(lParam);
			pkParam->y = (VeFloat32)HIWORD(lParam);
			pkParam->id = 0;
			g_pInputManager->AppendMessage(VeInputMessage::DT_TOUCH, VeInputMessage::TIT_BEGAN, g_pTime->GetTimeUInt(), pkParam);
			s_bTouched = true;
		}
		break;
	case WM_LBUTTONUP:
		{
			...
			pkParam->y = (VeFloat32)HIWORD(lParam);
			pkParam->id = 0;
			g_pInputManager->AppendMessage(VeInputMessage::DT_TOUCH, VeInputMessage::TIT_ENDED, g_pTime->GetTimeUInt(), pkParam);
			s_bTouched = false;
		}
		break;
	case WM_MOUSEMOVE:
		if(s_bTouched)
		{
			...
			pkParams->Clear();
			pkParams->Resize(1);
			pkParams->Front().x = (VeFloat32)LOWORD(lParam);
			pkParams->Front().y = (VeFloat32)HIWORD(lParam);
			pkParams->Front().id = 0;
			g_pInputManager->AppendMessage(VeInputMessage::DT_TOUCH, VeInputMessage::TIT_MOVED, g_pTime->GetTimeUInt(), pkParams);
		}
		break;
	case WM_CLOSE:
...
//--------------------------------------------------------------------------
VeMobileSim::VeMobileSim(VeStartParams& kParams)
	: VeDevice(TYPE_MOBSIM, kParams)
{

}
//--------------------------------------------------------------------------
VeMobileSim::~VeMobileSim()
{

}
//--------------------------------------------------------------------------
void VeMobileSim::_Init()
{
	InitDisplayModes();
}
//--------------------------------------------------------------------------
void VeMobileSim::_Term()
{
	TermDisplayModes();
}
//--------------------------------------------------------------------------
void VeMobileSim::_CreateWindow(Window* pkWindow)
{
	WNDCLASSEXA kClass;
...
//--------------------------------------------------------------------------
void VeMobileSim::_DestoryWindow(Window* pkWindow)
{
...
//--------------------------------------------------------------------------
void VeMobileSim::_SetWindowTitle(Window* pkWindow)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
...
//--------------------------------------------------------------------------
void VeMobileSim::_MinimizeWindow(Window* pkWindow)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
	::ShowWindow(pkData->m_hWnd, SW_MINIMIZE);
}
//--------------------------------------------------------------------------
void VeMobileSim::_RestoreWindow(Window* pkWindow)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
	pkData->m_bExpectedResize = TRUE;
...
//--------------------------------------------------------------------------
void VeMobileSim::_SetWindowBordered(Window* pkWindow, bool bBordered)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
...
//--------------------------------------------------------------------------
void VeMobileSim::_SetWindowFullscreen(Window* pkWindow,
...
//--------------------------------------------------------------------------
bool VeMobileSim::_SetWindowGammaRamp(Window* pkWindow,
...
//--------------------------------------------------------------------------
bool VeMobileSim::_GetWindowGammaRamp(Window* pkWindow,
	VeUInt16* pu16Ramp)
{
	VideoDisplay* pkDisplay = GetDisplayForWindow(pkWindow);
	VE_ASSERT(pkDisplay);
...
//--------------------------------------------------------------------------
void VeMobileSim::_SetWindowGrab(Window* pkWindow, bool bGrabbed)
{

}
//--------------------------------------------------------------------------
void VeMobileSim::_PumpEvents()
{
	MSG kMessage;
...
//--------------------------------------------------------------------------
void VeMobileSim::_EnableIME(Window* pkWindow)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
	if(pkData->m_hImc)
	{
...
//--------------------------------------------------------------------------
void VeMobileSim::_DisableIME(Window* pkWindow)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
	if(!pkData->m_hImc)
	{
...
//--------------------------------------------------------------------------
bool VeMobileSim::IMEHandleMessage(HWND hWnd, UINT uMessage, WPARAM wParam,
	LPARAM& lParam)
{
	bool bRes(false);
...
//--------------------------------------------------------------------------
void VeMobileSim::InitDisplayModes()
{
	DISPLAY_DEVICE kDevice;
...
//--------------------------------------------------------------------------
void VeMobileSim::TermDisplayModes()
{
	m_kDisplays.Clear();
}
//--------------------------------------------------------------------------
void VeMobileSim::_GetDisplayBounds(VideoDisplay& kDisplay, VeRect& kRect)
{
...
//--------------------------------------------------------------------------
void VeMobileSim::_GetDisplayModes(VideoDisplay& kDisplay)
{
...
//--------------------------------------------------------------------------
void VeMobileSim::SetupWindowData(Window* pkWindow, HWND hWnd,
	VE_BOOL bCreated)
{
	...
	pkData->m_hImc = ImmGetContext(hWnd);
...
#	ifdef GWLP_WNDPROC
	...
	if(pkData->m_pfuncWndProc == VeMobileSim::WndProc)
	{
		pkData->m_pfuncWndProc = NULL;
	}
	else
	{
		SetWindowLongPtr(hWnd, GWLP_WNDPROC, (LONG_PTR)VeMobileSim::WndProc);
	}
#	else
...
//--------------------------------------------------------------------------
void VeMobileSim::SetWindowPositionInternal(Window* pkWindow,
	VeUInt32 u32Flags)
{
	WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
	HWND hWnd = pkData->m_hWnd;
...
//--------------------------------------------------------------------------
void VeMobileSim::UpdateClipCursor(Window* pkWindow)
{
	//WindowData* pkData = (WindowData*)(VeRefObject*)pkWindow->m_spDriverData;
